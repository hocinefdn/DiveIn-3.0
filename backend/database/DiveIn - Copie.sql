...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 28, 2022 at 09:55 PM
...
-- Procedures
--
CREATE DEFINER=`divein31_root`@`localhost` PROCEDURE `add_notifications` (IN `id_user` INT, IN `id_post` INT)  BEGIN
  DECLARE a INT;
...
CREATE DEFINER=`divein31_root`@`localhost` PROCEDURE `get_recomandations` (IN `id_user` INT)  SELECT  users.id, users.email, users.phone_number, users.firstname, users.lastname, users.image FROM users,posts WHERE users.id = posts.id_user 
...
(11, 'amghis', 'kheloui', '2001-05-18', 0, '$2b$10$6V4jY1T6GhhcS4Bwjcas.e5O7pF6zUkJkQ6JsLchaUr4qPx9dQ3DO', '2019-12-31 23:00:00', NULL, 'ka@gmail.com', NULL, 'bio', '', NULL, '2022-01-07 17:26:11', NULL),
